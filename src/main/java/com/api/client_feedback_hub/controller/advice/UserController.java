...
//
    @PostMapping
    public CompletableFuture<ResponseEntity<String>> createUser(@RequestBody UserRegisterDto userRegisterDto) {
        DatabaseReference ref = FirebaseDatabase.getInstance().getReference("users");
        String uniqueUserId = ref.push().getKey();
        String hashedPassword = BCrypt.hashpw(userRegisterDto.getPassword(), BCrypt.gensalt());
        User newUser = new User(uniqueUserId, userRegisterDto.getEmail(), userRegisterDto.getName(),
                userRegisterDto.getPhoneNumber(), DEFAULT_ROLE, hashedPassword, 0.0);
